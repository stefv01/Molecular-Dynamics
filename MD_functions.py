...
class molecular_motion:
    ...
    def __init__(self, m, N, size, dim, r, temp, Dt, tsteps, dtype=np.float32):
        """
    ...
    def periodicBC(self, pos):
        ...
        pos = np.mod(pos, self.size / self.sigma, out=pos)
    ...
    def time_evol(self, init_pos, init_vel, method, units):
        ...
        pos = np.zeros((self.tsteps, self.N, self.dim), dtype=dtype) # positions
        vel = np.zeros((self.tsteps, self.N, self.dim), dtype=dtype) # velocities
        rel_pos = np.zeros((self.tsteps, self.N, self.N, self.dim), dtype=dtype) # relative positions
        rel_dist = np.zeros((self.tsteps, self.N, self.N), dtype=dtype) # relative distances
        energy_values = np.zeros((self.tsteps, 3), dtype=dtype)
    
        # Initialize the positions/velocities.
        pos[0] = init_pos.astype(dtype)
        vel[0] = init_vel.astype(dtype)
        # Import time step:
        Dt = self.Dt
        ...
        if method == 'euler':
            ...
            for t in tqdm(range(1, self.tsteps), desc = "Time Evolution (Euler)"):
                ...
                # Implement the Euler method equations
            ...
            for t in tqdm(range(1, self.tsteps), desc = "Time Evolution (Verlet)"):
                ...
                # Update relative positions/distances and forces for next iteration:
